				...
				for(var i=0; i<resultData.data.length; i++){
					...
					if(_data.errorList != undefined && _data.errorList.length > 0){
						for(var e = 0; e < _data.errorList.length; e++){
							...
							console.log(_curError);
							console.log(_err[_curError.errorField]);
...
function fnShowResult(){
	...
	for(var i=0; i<uploadResultDataList.length; i++){
		...
		if(_data.errorList != undefined && _data.errorList.length > 0){
			...
			_errCnt++;
			_errHtml += '		<tr>';
			_errHtml += '			<td>' + ComFns.convertText(_data.vo.sholSprtPrcnMngSn) + '</td>';
	...
	$.ajax({
		...
		success:function(resultData){
			...
			for(var i=0; i<resultData.length; i++){
				...
				html += 		(_err.sprtSn.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.sprtSn) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.sholSprtPrcnMngSn.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.sholSprtPrcnMngSn);
				html += 		(_err.sholSprtPrcnMngSn.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.sholSprtPrcnMngSn) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.slctnMthdCd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.slctnMthdCd);
				html += 		(_err.slctnMthdCd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.slctnMthdCd) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.slctnMmtCd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.slctnMmtCd);
				html += 		(_err.slctnMmtCd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.slctnMmtCd) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.fncrscCd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.fncrscCd);
				html += 		(_err.fncrscCd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.fncrscCd) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.giveYmd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.giveYmd);
				html += 		(_err.giveYmd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.giveYmd) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.sholGiveAmt.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.sholGiveAmt);
				html += 		(_err.sholGiveAmt.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.sholGiveAmt) + '</span>':'') ;
				html += '	</td>';
