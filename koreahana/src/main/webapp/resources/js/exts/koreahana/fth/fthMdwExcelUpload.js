	...
	$.ajax({
		...
		success:function(resultData){
			...
			for(var i=0; i<resultData.length; i++){
				...
				html += '	</td>';
				html += '	<td' + (_err.hanactPic.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.hanactPic);
				html += 		(_err.hanactPic.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.hanactPic) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.joinPeriodMmCnt.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.joinPeriodMmCnt);
				html += 		(_err.joinPeriodMmCnt.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.joinPeriodMmCnt) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.cncltnRsnCd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.cncltnRsnCd);
				html += 		(_err.cncltnRsnCd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.cncltnRsnCd) + '</span>':'') ;
				html += '	</td>';
