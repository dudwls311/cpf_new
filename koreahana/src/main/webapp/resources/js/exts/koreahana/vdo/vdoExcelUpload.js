	...
	$.ajax({
		...
		success:function(resultData){
			...
			for(var i=0; i<resultData.length; i++){
				...
				html += '	</td>';
				html += '	<td' + (_err.prtcrFlnm.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.prtcrFlnm);
				html += 		(_err.prtcrFlnm.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.prtcrFlnm) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.eduSprtTrprRelCd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.eduSprtTrprRelCd);
				html += 		(_err.eduSprtTrprRelCd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.eduSprtTrprRelCd) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.prtcrMbphno.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.prtcrMbphno);
				html += 		(_err.prtcrMbphno.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.prtcrMbphno) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.prtcrHanawonTh.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.prtcrHanawonTh);
				html += 		(_err.prtcrHanawonTh.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.prtcrHanawonTh) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.brplcCd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.brplcCd);
				html += 		(_err.brplcCd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.brplcCd) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.ntkrdfHanawonTh.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.ntkrdfHanawonTh);
				html += 		(_err.ntkrdfHanawonTh.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.ntkrdfHanawonTh) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.noExistBnfYn.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.noExistBnfYn);
				html += 		(_err.noExistBnfYn.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.noExistBnfYn) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.bf1ExistBnfYn.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.bf1ExistBnfYn);
				html += 		(_err.bf1ExistBnfYn.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.bf1ExistBnfYn) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.bf2ExistBnfYn.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.bf2ExistBnfYn);
				html += 		(_err.bf2ExistBnfYn.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.bf2ExistBnfYn) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.rcoblYn.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.rcoblYn);
				html += 		(_err.rcoblYn.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.rcoblYn) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.aplcntOgdp.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.aplcntOgdp);
				html += 		(_err.aplcntOgdp.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.aplcntOgdp) + '</span>':'') ;
				html += '	</td>';
				html += '</tr>';
			}
