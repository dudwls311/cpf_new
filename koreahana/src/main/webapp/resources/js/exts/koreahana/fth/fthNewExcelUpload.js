...
function fnShowResult(){
	...
	for(var i=0; i<uploadResultDataList.length; i++){
		...
		if(_data.errorList != undefined && _data.errorList.length > 0){
			...
			_errHtml += '			<td>' + ComFns.convertText(_data.vo.genderCd) + '</td>';
	...
	$.ajax({
		...
		success:function(resultData){
			...
			for(var i=0; i<resultData.length; i++){
				...
				html += '	</td>';
				html += '	<td' + (_err.ageCd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.ageCd);
				html += 		(_err.ageCd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.ageCd) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.ecnmatCd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.ecnmatCd);
				html += 		(_err.ecnmatCd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.ecnmatCd) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.crCd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.crCd);
				html += 		(_err.crCd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.crCd) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.emplisJoinYmd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.emplisJoinYmd);
				html += 		(_err.emplisJoinYmd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.emplisJoinYmd) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.salaryAmt.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.salaryAmt);
				html += 		(_err.salaryAmt.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.salaryAmt) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.dcsnSprtAmt.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.dcsnSprtAmt);
				html += 		(_err.dcsnSprtAmt.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.dcsnSprtAmt) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.entiscYmd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.entiscYmd);
				html += 		(_err.entiscYmd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.entiscYmd) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.trinsExpryYmd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.trinsExpryYmd);
				html += 		(_err.trinsExpryYmd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.trinsExpryYmd) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.savingDdlnYmd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.savingDdlnYmd);
				html += 		(_err.savingDdlnYmd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.savingDdlnYmd) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.prtprdExtsnCd.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.prtprdExtsnCd);
				html += 		(_err.prtprdExtsnCd.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.prtprdExtsnCd) + '</span>':'') ;
				html += '	</td>';
