...
<form:form commandName="jnitboarddbVO" name="bbsForm" id="bbsForm" method="post" enctype="multipart/form-data" action="${actionUrl }">
	...
	<table class="table_style" summary="<c:out value="${boardinfoVO.boardTitle}" /><%= JnitboardController.isLanguage("에 글을 작성하기 위한 입력폼으로",lang) %><c:if test="${boardinfoVO.useCategory == '1' && empty fixedCategory}"><c:out value="${lbl.category}" />, </c:if><c:if test="${boardinfoVO.useSecret == '1'}"><%= JnitboardController.isLanguage("비밀글 여부",lang) %>, </c:if><c:forEach var="k" items="${sIdx}" varStatus="s"><c:choose><c:when test="${k == 'writer' || k == 'hit'}"><c:if test="${isAdmin == true}"><c:out value="${f[k].lbl}" /></c:if></c:when><c:otherwise><c:out value="${f[k].lbl}" /></c:otherwise></c:choose><c:out value="${s.last ? '' : ','}" /></c:forEach><c:if test="${boardinfoVO.useFile == 1}">, <%= JnitboardController.isLanguage("첨부",lang) %></c:if> <%= JnitboardController.isLanguage("으로 구성되어 있습니다.",lang) %>">
		...
		<tbody>
			...
			<c:if test="${isAdmin == true && boardinfoVO.useNotice == '1'}">
				<tr>
					<th scope="row" style="width: 18%;"><%= JnitboardController.isLanguage("공지글",lang) %></th>
					<td style="text-align: left;"><input type="checkbox" class="st_check" name="isNotice" id="isNotice" value="1" /><label for="isNotice"></label></td>
				</tr>
			</c:if>
			<c:if test="${boardinfoVO.useSecret == '1' && loginVO.mbrId != null}">
				<tr>
					...
					<td style="text-align: left;"><input type="checkbox" name="isSecret" id="isSecret" value="1" /></td>
				</tr>
			</c:if>
			<c:forEach var="k" items="${sIdx}" varStatus="s">
				...
				<tr>
					...
					<td><input name="${k}" id="${k}" class="st_input input_long ${k}" type="text" value="<c:out value='${loginVO.mbrNm}'/>"/></td>
				</tr>
				</c:if>
				</c:when>
				<c:when test="${ k == 'hit'}">
				...
				<tr>
					<th scope="row"><label for="${k}"><c:out value="${f[k].lbl}" /></label></th>
					<td><input name="${k}" value="" id="${k}" class="st_input input_long ${k}" type="text" /></td>
				</tr>
				</c:if>
				</c:when>
				<%--created--%>
				...
				<tr>
					<th scope="row"><label for="${k}"><c:out value="${f[k].lbl}" /></label></th>
					<td><input name="${k}" id="${k}" class="input-xxlarge st_input input_long  ${k} datetime" type="text" data-datetype="yyyy-MM-dd hh:mm:ss"/></td>
				</tr>
				</c:if>
				</c:when>
				<%--title--%>
				...
				<tr>
					...
					<td><input name="${k}" value="" id="${k}" class="input-xxlarge st_input input_long ${k}" type="text" required/></td>
				</tr>
				<%--비회원작성자 --%>
				...
				<tr>
					...
					<td><input name="writer" id="writer" class="input-xxlarge st_input input_long ${k}" required type="text" value="<c:out value='${loginVO.mbrNm}'/>"/></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td colspan="2" class="board_td_content">
						<c:if test="${boardinfoVO.useWyswyg != 1}">
							<textarea id="${k}" class="${k} content st_input input_long" rows="10" name="${k}" required></textarea>
						</c:if>
						<c:if test="${boardinfoVO.useWyswyg == 1}">						
				...
				<tr>
					...
					<input name="${k}" id="${k}" value="" class="input-xxlarge st_input input_long ${k}" type="text" <c:out value="${f[k].required?' required':''}"/>>
					</c:when>
					<c:when test="${f[k].type == 'email'}">
					<input name="${k}" id="${k}" value="" class="input-xxlarge st_input input_long boardEmail ${k} " type="text" <c:out value="${f[k].required?' required':''}"/>>
					</c:when>
					<%--textarea--%>
					<c:when test="${f[k].type == 'textarea'}">
					<textarea id="${k}" rows="10" name="${k}" class="content ${k}" <c:out value="${f[k].required?' required':''}"/>></textarea>
					</c:when>
					<%--address--%>
					...
					<c:when test="${f[k].type == 'check'}">
							<c:forEach var="itm" items="${f[k].item}" varStatus="status">
							<input type="checkbox" name="${k}" id="${k}" value="${itm}" <c:out value="${f[k].required?' required':''}"/> ><label for="${k}">${itm}</label>
							</c:forEach>
					</c:when>
					<c:otherwise>
					${f[k].type}<input name="${k}" id="${k}" value="" class="st_input input_long ${k}" type="text" <c:out value="${f[k].required?' required':''}"/>>
					</c:otherwise>
					</c:choose>
					</td>
				</tr>
	...
	<div class="btn_g_btm">
		<button id="btnSubmit" class="btn_st btn_c_gr btn_s_big"><%= JnitboardController.isLanguage("등록",lang) %></button>
		<a id="linkCancel" class="btn_st btn_s_big" href="?boardId=${boardinfoVO.boardId}&amp;mode=list&amp;category=${category}" ><%= JnitboardController.isLanguage("취소",lang) %></a>
	</div>
