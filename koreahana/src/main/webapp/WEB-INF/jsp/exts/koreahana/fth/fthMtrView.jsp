...
<script type="text/javascript" src="/resources/js/exts/koreahana/fth/fthMtrView.js"></script>
...
<form id="listPageForm" action="?">
	...
	<input type="hidden" name="pageIndex" value="<c:out value="${param.pageIndex }" />" />
					...
					<table class="table_style table_t_left th_v_m">
						...
						<tr>
							<th><label class="MAL0" for="flnm">성명</label> </th>
							<td>
					...
					<table class="table_style table_t_left th_v_m">
						...
						<tr>
							<th scope="row"><label class="MAL0" for="dcsnSprtAmt">사용용도</label> </th>
							<td colspan="3">
								<c:out value="${jtag:getCdNm(usdusgCdList,result.usdusgCd)}" />
							</td>
						</tr>
						<tr>
							<th scope="row"><label class="MAL0" for="idtprsSavingAmt">본인적립금(총액기준)</label> </th>
							<td>
								<fmt:formatNumber value="${result.idtprsSavingAmt}" /> 원					
							</td>
							<th scope="row"><label class="MAL0" for="fndtSavingAmt">재단적립금(지급액기준)</label> </th>
							<td>
								<fmt:formatNumber value="${result.fndtSavingAmt}" /> 원					
							</td>
						</tr>
						<tr>
							<th scope="row"><label class="MAL0" for="mtryMmCnt">만기</label> </th>
							<td>
								<c:out value="${result.mtryMmCnt}" />	개월
							</td>
							<th scope="row"><label class="MAL0" for="fncEduTkclsYmd">금융교육수강일</label> </th>
							<td>
								<c:out value="${jtag:convertDateSplitString(result.fncEduTkclsYmd,'-')}" />	
							</td>
						</tr>
						<tr>
