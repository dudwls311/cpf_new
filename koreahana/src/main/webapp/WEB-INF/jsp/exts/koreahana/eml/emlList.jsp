...
<script type="text/javascript" src="/resources/js/exts/koreahana/eml/emlList.js"></script>
			...
			<c:forEach var="result" items="${resultList }" varStatus="status">
				...
				<tr>
					...
					<td class="td_bg">
						...
						<c:if test="${result.sprtSttsCd == enumSprUndCd && isCenterStaff == false }">
							&nbsp;&nbsp;<a href="#" class="btn_st btn_c_re" onclick="fnRsnChg(<c:out value="'${result.sprtSn}'" />);return false;">사유등록</a>
						</c:if>
						<c:if test="${result.sprtSttsCd == enumSprUndCd && isCenterStaff == true }">
							<a href="#none" class="btn_st btn_c_re" onclick="fnEmlRsnShow(<c:out value="'${result.pbancrcSn}','${result.pbancrcCtgryFrstCd}','${result.sprtSn}'" />);return false;">사유확인</a>
						</c:if>
					</td>
				</tr>
			</c:forEach>
