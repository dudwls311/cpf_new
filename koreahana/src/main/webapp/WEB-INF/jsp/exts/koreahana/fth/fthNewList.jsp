...
<script type="text/javascript" src="/resources/js/exts/koreahana/fth/fthNewList.js"></script>
...
<form id="writePageForm" action="?">
	...
	<input type="hidden" id="kfnMode" name="kfnMode" value="" />
	<input type="hidden" id="fthpbbNewAplySn" name="fthpbbNewAplySn" value="" />
</form>
		...
		<div class="ig_wrap">
			...
			<div class="con_b_tp">
				...
				<c:if test="${hideWrite != 'Y' && isStreAuth == true}">
					<a class="btn_st btn_c_bk" href="#" onclick="fnWrite('');return false;">신규신청정보 개별등록</a>
					<a class="btn_st btn_c_bk" href="#" onclick="fnWriteBundle();return false;">신규신청정보 일괄등록</a>
				</c:if>
			</div>
				...
				<table class="table_style thd_v_m">
					...
					<c:forEach var="result" items="${resultList}" varStatus="status">
						...
						<tr>
							...
							<td><a href="#" onclick="javascript:fnView('<c:out value="${result.fthpbbNewAplySn}" />');return false;"><c:out value="${result.flnm}" /></a></td>
							<td><c:out value="${result.jrdcHanactNm}" /></td>
							<td><fmt:formatNumber value="${result.dcsnSprtAmt}" /></td>
							<td><c:out value="${jtag:convertDateSplitString(result.bbJoinYmd,'-')}" /></td>
							<td><c:out value="${jtag:convertDateSplitString(result.savingDdlnYmd,'-')}" /></td>
						</tr>
					</c:forEach>
					</tbody>
				</table>
