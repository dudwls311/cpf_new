...
public class JnitcmsorgServiceImpl extends EgovAbstractServiceImpl implements
    ...
    /**
	 * koreahana에서  하나센터 조회용도
	 * @param searchVO - 조회할 정보가 담긴 VO
	 * @return JNITCMSORG 목록
	 * @exception Exception
	 */
    public List<JnitcmsorgVO> selectHanactList(JnitcmsorgDefaultVO searchVO) {
    	List<String> orgIdNotInList = new ArrayList<String>();
    	orgIdNotInList.add("20002");		//사이트관리
    	orgIdNotInList.add("20003");		//기타
    	searchVO.setOrgIdNotInList(orgIdNotInList);
        return jnitcmsorgDAO.selectHanactList(searchVO);
    }
}
