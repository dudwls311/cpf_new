package exts.koreahana.mbr.validator;
...
/**
 * @Class Name : koreahanaMbrValidator.java
 * @Description : 회원 가입,수정 Validator
 ...
 */
@Component("koreahanaMbrValidator")
public class KoreahanaMbrValidator extends ExtsAbstractValidator implements Validator {
	//validate체크할  field들
	...
	public void validate(final Object obj, final Errors errors) {
		...
		if(EnumMbrTypeCd.NTK.getCode().equals(command.getTypeId())||EnumMbrTypeCd.NOR.getCode().equals(command.getTypeId())) {
			...
			if(EnumMbrTypeCd.NTK.getCode().equals(command.getTypeId())) {
				ValidationUtils.rejectIfEmptyOrWhitespace(errors,rrno, REQUIRED_FIELD, getMessage("exts.error.koreahana.mbr.nocertification"));			
				ValidationUtils.rejectIfEmptyOrWhitespace(errors,ntkrdfUnqNo, REQUIRED_FIELD, getMessage("exts.error.koreahana.mbr.nocertification"));
