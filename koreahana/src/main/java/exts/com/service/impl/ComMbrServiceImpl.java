...
public class ComMbrServiceImpl extends ExtsAbstractServiceImpl implements ComMbrService {
	...
	*/
	...
	//        			throwBizException("com.error.login.wrongpw", new String[] {String.valueOf(pwMiscnt),Globals.LOGIN_PW_MISS_MAX_COUNT});
    	...
    	log.debug("1111111");
    	//로그인 2차인증(일반사용자를 제외한 나머지 SMS 인증 필요)
    	if("Y".equals(Globals.SMS_AUTH_USE_YN)) {
    		log.debug("2222222");
    		boolean isNormalUser = (resVO.getTypeId().equals(EnumMbrTypeCd.NTK.getCode()) || resVO.getTypeId().equals(EnumMbrTypeCd.NOR.getCode()) ? true : false);
    		log.debug("isNormalUser");
			if(!isNormalUser) {
				log.debug("33333333333");
				if("Y".equals(searchVO.getSmsAuthNumberReqYn())) {	//인증번호 전송(hidden 파라미터로 전달받음)
					log.debug("444444444");
					String smsRet = smsAuthLogin(comMbrVO.getMbphno());
			    	if(smsRet != null) return smsRet;	//
				}else {		//인증번호 확인
					log.debug("55555555");
					smsAuthLoginCheck(searchVO);
				}
			}
    	}
...
//					lmenuList.add(makeMenuVO(tmpVO, umaList, dmaList));
		...
		SessionUtil.setAttribute(SESSION_MY_MAIN, "/exts/koreahana/com/main.do");//메인페이지 처리.		
		if("".equals(ret))ret = "/exts/koreahana/com/main.do";
...
//		if(menuVO.isRedngAuth() || menuVO.isDelAuth() || menuVO.isPrntgAuth())menuVO.setStreAuth(true);
	 ...
	 * @throws EgovBizException
	 */
	private String smsAuthLogin(String targetNumber) throws EgovBizException {
		if("".equals(NullUtil.nullString(targetNumber))) throwBizException("exts.error.koreahana.mbr.smsLoginTargetNumberNotFound");
		
		DateTime sessionSmsAuthTime = (DateTime)SessionUtil.getAttribute("SMS_AUTH_TIME");
		int smsSendTimeDiff = Integer.parseInt(Globals.SMS_AUTH_REQUEST_SECOND);
		if(sessionSmsAuthTime != null && Seconds.secondsBetween(sessionSmsAuthTime, new DateTime()).getSeconds() < smsSendTimeDiff) {
			throwBizException("exts.error.koreahana.mbr.smsAuthTimeCheck", new String[] {smsSendTimeDiff+""});
		}
		
		Random random = new Random();
		String rndNum = String.format("%06d", random.nextInt(999999));
		SessionUtil.setAttribute("SMS_AUTH_NUMBER", rndNum);
		SessionUtil.setAttribute("SMS_AUTH_TIME", new DateTime());
		
		CielSmsVO smsVO = new CielSmsVO();
		smsVO.setTargetNumber(targetNumber);
		smsVO.setSendNumber(Globals.SMS_AUTH_SEND_NUMBER);
		smsVO.setSendTitle(Globals.SMS_AUTH_SEND_TITLE);
		smsVO.setSendMessage(getMessage("exts.error.koreahana.mbr.smsAuthText", new String[] {rndNum}));
		if(!cielSmsService.sendSms(smsVO)) throwBizException("exts.item.koreahana.pba.smsSendError");
	 ...
	 * @throws EgovBizException
	 */
	private void smsAuthLoginCheck(ComMbrVO searchVO) throws EgovBizException {
		String smsAuthNumber = NullUtil.nullString(searchVO.getSmsAuthNumber());
		String sessionSmsAuthNumber = (String)SessionUtil.getAttribute("SMS_AUTH_NUMBER");
		DateTime sessionSmsAuthTime = (DateTime)SessionUtil.getAttribute("SMS_AUTH_TIME");
		
		if(smsAuthNumber.equals(sessionSmsAuthNumber)) {
			if(Seconds.secondsBetween(sessionSmsAuthTime ,new DateTime()).getSeconds() < Integer.parseInt(Globals.SMS_AUTH_INVALID_SECOND)) {
				SessionUtil.removeAttribute("SMS_AUTH_NUMBER");
				SessionUtil.removeAttribute("SMS_AUTH_TIME");
			}else {
				throwBizException("exts.error.koreahana.mbr.smsAuthInvalidTimeout");
			}
		}else {
			throwBizException("exts.error.koreahana.mbr.diffSmsAuthNumber");
		}
	}
