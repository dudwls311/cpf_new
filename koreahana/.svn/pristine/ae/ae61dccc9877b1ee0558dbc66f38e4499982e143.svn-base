...
//		storePathString += System.getProperty("file.separator") + sdf.format(new Date());
     ...
     * @throws Exception
     */
    public static void downFileNoneEnc(HttpServletRequest request, HttpServletResponse response, String fileStoreCours, String fileStoreName, String fileOriginalName) throws Exception {
    	downFileNoneEnc(request, response, fileStoreCours, fileStoreName, fileOriginalName, false);
    }
     ...
     * @param fileStoreName
     * @param fileOriginalName
     * @param isDecFileDelete
     * @throws Exception
     */
    public static void downFileNoneEnc(HttpServletRequest request, HttpServletResponse response, String fileStoreCours, String fileStoreName, String fileOriginalName, boolean isDecFileDelete) throws Exception {
...
//			DownloadUtil.setDisposition(fVO.getFileOriginalName(), request, response);
			...
			try {
			    ...
			    if(isDecFileDelete && file.exists()) file.delete();		//복호화된 파일 제거
			}
    ...
    public static void downFile(HttpServletRequest request, HttpServletResponse response, String fileStoreCours, String fileStoreName, String fileOriginalName) throws Exception {
    	File file = ComFileUploadUtil.getDecFile(fileStoreCours, fileStoreName);
    	downFileNoneEnc(request, response, file.getParent(), file.getName(), fileOriginalName, true);
    }
    ...
    /**
     * 기본파일저장경로를 포함한 파일다운로드처리.(DB컬럼에 저장하는 경로가 없는경우) DRM
     */
    public static void downDrmFile(HttpServletRequest request, HttpServletResponse response, String fileStoreCours, String fileStoreName, String fileOriginalName, String fileBasePath) throws Exception {
    	String storePathString = fileBasePath;//기본경로	
		if (fileStoreCours != null && !"".equals(fileStoreCours)) {
		    storePathString += System.getProperty("file.separator") + fileStoreCours;
		}
     ...
     * @throws Exception
     */
    public static void showFileNoneEnc(HttpServletRequest request, HttpServletResponse response, String fileStoreCours, String fileStoreName) throws Exception {
		...
		try {
			...
			if(file.exists()) file.delete();		//복호화된 파일 제거
		}
    }
    /**
    ...
    public static void showFile(HttpServletRequest request, HttpServletResponse response, String fileStoreCours, String fileStoreName, String fileBasePath) throws Exception {
		...
		File file = ComFileUploadUtil.getDecFile(storePathString, fileStoreName);
		showFileNoneEnc(request, response, file.getParent(), file.getName());
    }

    /**
     * 기본파일저장경로를 포함한 이미지보여주기.(DB컬럼에 저장하는 경로가 없는경우)
     */
    public static void showFileNoneEnc(HttpServletRequest request, HttpServletResponse response, String fileStoreCours, String fileStoreName, String fileBasePath) throws Exception {
    	String storePathString = fileBasePath;//기본경로	
		if (fileStoreCours != null && !"".equals(fileStoreCours)) {
		    storePathString += System.getProperty("file.separator") + fileStoreCours;
		}
		showFileNoneEnc(request, response, storePathString, fileStoreName);
    }
