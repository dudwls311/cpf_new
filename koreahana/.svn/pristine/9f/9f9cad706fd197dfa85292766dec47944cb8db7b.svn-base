	...
	$.ajax({
		...
		success:function(resultData){
			...
			for(var i=0; i<resultData.length; i++){
				...
				html += '	</td>';
				html += '	<td' + (_err.frscpkEdu.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.frscpkEdu);
				html += 		(_err.frscpkEdu.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.frscpkEdu) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.newYn.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.newYn);
				html += 		(_err.newYn.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.newYn) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.sprtCycl.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.sprtCycl);
				html += 		(_err.sprtCycl.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.sprtCycl) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.sprtDcsnAmt.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.sprtDcsnAmt);
				html += 		(_err.sprtDcsnAmt.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.sprtDcsnAmt) + '</span>':'') ;
				html += '	</td>';
				...
				html += '	</td>';
				html += '	<td' + (_err.noExeAmt.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.sprtDcsnAmt - _data.vo.sprtAmt);
				html += 		(_err.noExeAmt.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.noExeAmt) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.prchsBzenty.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.prchsBzenty);
				html += 		(_err.prchsBzenty.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.prchsBzenty) + '</span>':'') ;
				html += '	</td>';
				html += '	<td' + (_err.prchsItem.length > 0?' class="error"':'') + '>' ;
				html += 		ComFns.convertText(_data.vo.prchsItem);
				html += 		(_err.prchsItem.length > 0?'<span class="tooltip">' + ComFns.printErrorArrayMessage(_err.prchsItem) + '</span>':'') ;
				html += '	</td>';
