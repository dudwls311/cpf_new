...
<div class="member_content">
	...
	<form id="idFindForm" action="?" method="post" >
		...
		<div class="findGroup">
			<div class="findForm">
				<p class="findid01 btn_g_btm AlignCenter">
					<c:if test="${siauth == true}" >
						<a href="javascript:authSiIdfind();" class="btn_st btn_s_big btn_c_bl" style="margin: 0 auto;">실명인증</a>
					</c:if>
					<c:if test="${gpinauth == true}" >
						<a href="javascript:authGpinIdfind();" class="btn_st btn_s_big btn_c_gr" style="margin: 0 auto;">I-PIN인증</a>
					</c:if>
				</p>
				<!--결과-->
