<% response.setHeader("Cache-Control","no-store"); response.setHeader("Pragma","no-cache"); response.setDateHeader("Expires",0); if (request.getProtocol().equals("HTTP/1.1")) { response.setHeader("Cache-Control", "no-cache"); } %><%@ page contentType="text/html; charset=utf-8" %><%@ include file='/_common/header.jsp' %><c:set var="Jnit_sitePath" value="support" /><%@ page import="jnit.cms.handler.*" %><%@ page import="jnit.com.util.CclUtil" %><%@ page import="jnit.cms.menu.JnitcmsmenuController" %><c:set var="Jnit_sitePath" value="${Jnit_sitePath}" scope="request"/><% /* cset_s */ %><% String servletPath = request.getParameter("path"); if(servletPath.lastIndexOf("index.jsp") == -1){ servletPath += "index.jsp";	} %><%@ page import="egovframework.com.utl.fcc.service.StringUtil"%><% String localYn = ""; if(StringUtil.isExistString(request.getRequestURL().toString(),"local")) localYn="Y"; %><c:set var="localYn" value="<%= localYn %>" scope="request" /><% /* cset_e */ %><% /*<!-- [-CONTENT-] -->*/ %><%
/**
 *@version 3.2.0.1
 **/
%>
			...
			<div class="print_content">
				<div class="print">
					...
					<a href="#" onclick="javascript:window.close();">
						<img src="${pageContext.request.contextPath }/support/common/img/scrap_del.gif" alt="인쇄 닫기" width="26px"/>
					</a>
				</div>
