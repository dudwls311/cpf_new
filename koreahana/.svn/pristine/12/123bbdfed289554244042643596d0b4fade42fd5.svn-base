 ...
 * 제출서류 첨부파일폼 추가
 */
ComFns.smbsnDocFileFormAdd = function(prefix, docBoxYn, smbsnDocMtlYn){
	var fileFormObj = $('[id^='+prefix+'][id$=Form]');
	var lastFileFormObj = fileFormObj.last();
	var maxSort = Number(lastFileFormObj.data(prefix+'sort'));
	if(isNaN(maxSort)) maxSort = 0;
	var nextSort = maxSort+1;
	var subPrefix = prefix+nextSort+'_';
	
	var html = fnSmbFileTdForm(nextSort, prefix, subPrefix, '', docBoxYn, smbsnDocMtlYn, null);
	if(fileFormObj.length == 0){
		$('[id='+prefix+']').append(html);
	}else{
		lastFileFormObj.after(html);
	}
	
	//첨부파일 클릭 트리거
	$('#'+subPrefix+'Form').find("a[id$=Trg]").bind('click', function(){
	...
	//첨부파일명 설정
	$('#'+subPrefix+'Form').find("input[type=file]").bind('change', function(){
	...
	//첨부파일초기화
	$('#'+subPrefix+'Form').find("a[id$=Delete]").bind('click', function(){
 ...
 * 제출서류 첨부파일폼 제거
 */
ComFns.smbsnDocFileFormDel = function(type, id){
	var fileFormObj = $('[id^='+type+'][id$=Form]');
	if(fileFormObj.length < 2){
		alert('더이상 제거가 불가능 합니다.');
	}else{
		$('#'+id+'Form').remove();
	}
}
 ...
 * 제출서류 첨부파일 삭제
 */
ComFns.smbsnDocFileDelAct = function(prefix, subPrefix, docBoxYn, smbsnDocMtlYn, mpnSn, isAddForm){
	if(isAddForm == undefined || isAddForm == null) isAddForm = true;
	var fileFormObj = $('#'+subPrefix+'Form');
	fileFormObj.remove();
	...
	if(isAddForm){
		if($('[id^='+prefix+'][id$=Form]').length < 1){
			ComFns.smbsnDocFileFormAdd(prefix, docBoxYn, smbsnDocMtlYn);
		}
	}
}
 ...
 * 나의문서함(제출서류용)
 */
ComFns.openMyDocBox = function(smbsnDocPrefix, smbsnDocMtlYn, smbDocMpngSn){
	var pageIndex = 1;
	ComFns.popup.init({title:'나의 문서함'});
	ComFns.popup.loadContent(ComFns.getContextUrl() + 'user/exts/koreahana/doc/index.do', {kdMode:'listAjax', docBoxSelectType:'ComFns.docBoxSelect', smbsnDocPrefix:smbsnDocPrefix, smbsnDocMtlYn:smbsnDocMtlYn, smbDocMpngSn:smbDocMpngSn, pageIndex:pageIndex} );
	ComFns.popup.show();
}
 ...
 * 나의문서함>문서선택(제출서류용)
 */
ComFns.docBoxSelect = function(docId){
	var docObj = $("#"+docId);
	var smbsnDocPrefix = docObj.data('smbsndocprefix');
	var smbsnDocMtlYn = docObj.data('smbsndocmtlyn');
	var smbDocMpngSn = docObj.data('smbdocmpngsn');
	var docBoxSn = docObj.data('docboxsn');
	var atchFileSn = docObj.data('atchfilesn');
	var orgnlAtchFileNm = docObj.data('orgnlatchfilenm');

	$("#"+smbsnDocPrefix+'Div').show();
	$("#"+smbsnDocPrefix+'fsn').val(atchFileSn);
	$("#"+smbsnDocPrefix+'FileNm').empty().text(orgnlAtchFileNm);
	ComFns.popup.hide();
}
 ...
 * 나의문서함(지원첨부파일용)
 */
ComFns.openMyDocBox2 = function(smbsnDocPrefix){
	var pageIndex = 1;
	ComFns.popup.init({title:'나의 문서함'});
	ComFns.popup.loadContent(ComFns.getContextUrl() + 'user/exts/koreahana/doc/index.do', {kdMode:'listAjax', docBoxSelectType:'ComFns.docBoxSelect2', smbsnDocPrefix:smbsnDocPrefix, pageIndex:pageIndex} );
	ComFns.popup.show();
}
 ...
 * 나의문서함>문서선택(지원첨부파일용)
 */
ComFns.docBoxSelect2 = function(docId){
	var docObj = $("#"+docId);
	var smbsnDocPrefix = docObj.data('smbsndocprefix');
	var atchFileSn = docObj.data('atchfilesn');
	var orgnlAtchFileNm = docObj.data('orgnlatchfilenm');
	...
	$("#"+smbsnDocPrefix+'Fsn').val(atchFileSn);
	$("#"+smbsnDocPrefix+'FileFileNm').empty().text(orgnlAtchFileNm);
	ComFns.popup.hide();
}
 ...
 * 서명
 */
ComFns.openMySgn = function(sgnId){
	if(sgnId == undefined || sgnId == null || sgnId == '') sgnId = 'sgntFileSn';
	
	ComFns.popup.init({title:'서명 선택'});
	ComFns.popup.loadContent(ComFns.getContextUrl() + 'user/exts/koreahana/sgn/index.do', {ksMode:'listAjax',sgnId:sgnId} );
	ComFns.popup.setConfirmBtnClickMethod(ComFns.sgnSelect);
 ...
 * 서명선택
 */
ComFns.sgnSelect = function(){
	var sgnCheckObj = $("input[type=radio][id^=sgnRadio]:checked");
	if(sgnCheckObj.length < 1){
		alert('서명파일을 선택해주세요.');
		return false;
	}
	var url = sgnCheckObj.data('url');
	var param = sgnCheckObj.data('param');
	var fsn = sgnCheckObj.data('fsn');
	var sgntNm = sgnCheckObj.data('sgntnm');
	var sgnId = sgnCheckObj.data('sgnid');
 ...
 * 서명로드
 */
ComFns.sgnLoad = function(sgnId){
	if(sgnId == undefined || sgnId == null || sgnId == '') sgnId = 'sgntFileSn';
	var sgnObj = $("#"+sgnId+"Hidden");
	if(sgnObj.length == 0)  return false;
	
	var url = sgnObj.data('url');
	var param = sgnObj.data('param');
	var fsn = sgnObj.data('fsn');
 ...
 * 모바일 업로드 생성
 */
ComFns.openMblPopup = function(callback){
	var _host = window.location.protocol + '//' + window.location.host;
	ComFns.popup.init({title:'모바일파일업로드'});
	ComFns.popup.loadContent(ComFns.getContextUrl() + 'exts/koreahana/mbl/showQrPopupAjax.do', {callback:callback,host:_host} );
	ComFns.popup.show();
}
