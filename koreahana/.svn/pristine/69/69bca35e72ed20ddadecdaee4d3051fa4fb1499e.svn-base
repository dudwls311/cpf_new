...
<script type="text/javascript" src="/resources/js/exts/koreahana/fth/fthMdwList.js"></script>
...
<form id="writePageForm" action="?">
	...
	<input type="hidden" id="fthpbbMdwCncltnSn" name="fthpbbMdwCncltnSn" value="" />
</form>
		...
		<div class="ig_wrap">
			...
			<div class="con_b_tp">
				...
				<c:if test="${hideWrite != 'Y' && isStreAuth == true}">
					<a class="btn_st btn_c_bk" href="#" onclick="fnWrite('');return false;">중도해지정보 개별등록</a>
					<a class="btn_st btn_c_bk" href="#" onclick="fnWriteBundle();return false;">중도해지정보 일괄등록</a>
				</c:if>
			</div>
				...
				<table class="table_style thd_v_m">
					<thead>
						<tr>
							...
							<th scope="col">본인적립금</th>
							<th scope="col">재단적립금</th>
						</tr>
					</thead>
					<tbody>
					<c:forEach var="result" items="${resultList}" varStatus="status">
						...
						<tr>
							...
							<td><a href="#" onclick="javascript:fnView('<c:out value="${result.fthpbbMdwCncltnSn}" />');return false;"><c:out value="${result.flnm}" /></a></td>
							<td><c:out value="${result.jrdcHanactNm}" /></td>
							<td><c:out value="${jtag:convertDateSplitString(result.bbJoinYmd,'-')}" /></td>
							<td><c:out value="${result.joinPeriodMmCnt}" />개월</td>
