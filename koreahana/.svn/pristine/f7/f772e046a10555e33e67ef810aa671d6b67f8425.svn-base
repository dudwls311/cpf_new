<% response.setHeader("Cache-Control","no-store"); response.setHeader("Pragma","no-cache"); response.setDateHeader("Expires",0); if (request.getProtocol().equals("HTTP/1.1")) { response.setHeader("Cache-Control", "no-cache"); } %><%@ page contentType="text/html; charset=utf-8" %><%@ include file='/_common/header.jsp' %><c:set var="Jnit_sitePath" value="support" /><%@ page import='java.util.Properties' %><%@ page import='net.sf.json.JSONObject' %><%@ page import='jnit.cms.config.ConfigUtil' %><%	JSONObject prop = new JSONObject(); try{prop = ConfigUtil.getProp("SITE_00001"); /*필수 입력 siteId*/ }catch(Exception e){/*log.error(e.getMessage());*/} String error = (String)prop.get("error"); %><%@ page import="jnit.scrap.ScrapUtil" %><%@ page import="jnit.login.log.loginlogUtil" %><%@ page import="java.util.ArrayList" %><%@ page import="egovframework.rte.ptl.mvc.tags.ui.pagination.PaginationInfo" %><% String scrapPageNo = request.getParameter("scrapPageNo");String logPageNo = request.getParameter("logPageNo");String tabs = request.getParameter("tabs");String tab = request.getParameter("tab");int scrapStartNum = ScrapUtil.scrapStartNum(scrapPageNo);int logStartNum = loginlogUtil.listIPStartNum(logPageNo);ArrayList scrapList = new ArrayList<String>();scrapList = ScrapUtil.scrapList(scrapPageNo);PaginationInfo scrapPageNavi = new PaginationInfo();scrapPageNavi = ScrapUtil.scrapPageNavi(scrapPageNo);ArrayList loginLogList = new ArrayList<String>();loginLogList = loginlogUtil.listIP(logPageNo);PaginationInfo loginlogPageNavi = new PaginationInfo();loginlogPageNavi = loginlogUtil.pageNavi(logPageNo); String errorCode = ConfigUtil.getErrorCode("support","myPage","G","SITE_00001");%><%@ page import="jnit.com.util.DBType" %><%@ include file='/support/_tpls/B/TPL_000018_S.jsp' %><% /*<!-- [-CONTENT-] -->*/ %><%
 ...
 //-->
    ...
    <div class="tabbable black-box" style="margin-top: 20px;margin-bottom: 18px;">
      ...
      <ul class="tab_st01 MAB30" id="nav">
      ...
      <div class="tab-content">
        <div class="tab-pane" id="tab1">
          <div class="padded">
            <div class="member_join_form AlignLeft">
              <c:if test="${loginVO.typeId != '30006'}" >
                <form name="mbrForm" id="mbrForm" action="<c:url value='/mbr/modify_savembr.do'/>" method="post" onsubmit="$('#passwd').val($('#passwd').val().replace(/&/g, '*')); $('#passwdConfirm').val($('#passwdConfirm').val().replace(/&/g, '*')); return true;">
                  ...
                  <input type="text" id="mbrNm" name="mbrNm" value="${loginVO.mbrNm}" class="st_input input_long MAB20"/>
                  <div class="mypage_wrap  st_select st_radio"style="width: 100%;">
                    <div class="st_input">
                      <c:if test="${prop['tel'] != 0}" >
                        <c:if test="${prop['tel']  == 2}"><font color="red"> (필수) </font></c:if>
                    ...
                    <div class="st_input">
                      <c:if test="${prop['phone'] != 0}" >
                        <c:if test="${prop['phone']  == 2}"><font color="red"> (필수) </font></c:if>
                    ...
                    <div class="st_input">
                      <c:if test="${prop['email'] != 0}" >
                        <c:if test="${prop['email']  == 2}"><font color="red"> (필수) </font></c:if>
                        <label for="email">이메일</label>
                        <c:if test="${prop['email'] != 0}" >
                          <c:if test="${prop['email'] == 2}">
                            <c:set var="configEmail" value="${prop['email'] == '2' ? 'required' : '' }"/></c:if>
                          <input type="text" name="email" id="email" value="${loginVO.email}" ${configEmail} />
                          <c:if test="${prop['email']  == 2}">
                            <font color="red"> * </font>
                          </c:if>
                        </c:if>
                    ...
                    <div class="st_input">
                      <c:if test="${prop['post'] != 0}" >
                        <c:if test="${prop['post']  == 2}"><font color="red"> (필수) </font></c:if>
                    ...
                    <div class="st_input">
                      <c:if test="${prop['addr'] != 0}" >
                        <c:if test="${prop['addr']  == 2}"><font color="red"> (필수) </font></c:if>
                        <label for="addr">주소</label>
                        <c:if test="${prop['addr'] != 0}" >
                          <c:if test="${prop['addr'] == 2}">
                            <c:set var="configAddr" value="${prop['addr'] == '2' ? 'required' : '' }"/>
                          </c:if>
                          <input type="text" name="addr" id="addr" ${configAddr}/>
                          <c:if test="${prop['addr']  == 2}"><font color="red"> * </font>
                          </c:if>
                        </c:if>
                      </c:if>
                    ...
                    <div class="st_input">
                      <c:if test="${prop['emailRecv'] != 0}" >
                        <c:if test="${prop['emailRecv']  == 2}"><font color="red"> (필수) </font></c:if>
                        <label for="emailRecv">메일수신동의</label>
                    ...
                    <div class="st_input">
                      <c:if test="${prop['smsRecv'] != 0}" >
                        <c:if test="${prop['smsRecv']  == 2}"><font color="red"> (필수) </font></c:if>
                        <label for="smsRecv">SMS수신동의</label>
                    ...
                    <div class="st_input">
                      <c:if test="${prop['ex01Add'] != 0}">
                        <c:if test="${prop['ex01Add']  == 2}"><font color="red"> (필수) </font></c:if>
                        ${prop['ex01']}
                    ...
                    <div class="st_input">
                      <c:if test="${prop['ex02Add'] != 0}">
                        <c:if test="${prop['ex02Add']  == 2}"><font color="red"> (필수) </font></c:if>
                        ${prop['ex02']}
                    ...
                    <div class="st_input">
                      <c:if test="${prop['ex03Add'] != 0}">
                        <c:if test="${prop['ex03Add']  == 2}"><font color="red"> (필수) </font></c:if>
                        ${prop['ex03']}
                    ...
                    <div class="st_input">
                        <label for="passwd"><strong>비밀번호</strong></label>
                    ...
                    <div class="st_input">
                        <label for="passwdConfirm"><strong>비밀번호확인</strong></label>
                      
                        <input type="password" id="passwdConfirm" name="passwdConfirm"/>
                      <div id="pawConfirm"><strong>영문, 숫자8~12자 이내로 입력해주시기 바랍니다.</strong></div>
                    </div>

                  </div>
                  <c:if test="${loginVO != null}">
                    <div class="btn_g_btm MAB20">
                    <button type="submit" class="btn_st btn_c_gr">정보수정</button>
                    <button id="memberCancel" class="btn_st btn_c_bk">회원탈퇴</button>
                    <button id="sginupCel" class="btn_st ">취소</button>
                  </div>
                  </c:if>
                </form>
              </c:if>
          ...
          <div class="tab-pane" id="tab2">
            <div class="padded">
              <div class="member_join_form">
                <table class="table_style" summary="스크랩 정보 페이지입니다.">
                    ...
                    <c:if test="${!empty scrapList }">
                      <c:forEach var="result" items="${scrapList}" varStatus="status">
                        <tr rel="${result.scrapId}">
                          ...
                          <td><a href="javascript:delScrap('${result.scrapId}');"><img src="${pageContext.request.contextPath }/support/common/img/scrap_del.gif" alt="스크랩 삭제" /></a></td>
                        </tr>
                      </c:forEach>
                    </c:if>
                    <c:if test="${empty scrapList }">
          ...
          <div class="tab-pane" id="tab3">
            <div class="padded">
              <div class="member_join_form">
                <table class="table_style"summary="로그인 정보 페이지입니다.">
